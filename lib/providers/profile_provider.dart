...
Future<String> _decryptValue(String? encrypted) async {
  ...
  // GCM 升级后旧 enc: 前缀数据无法解密，返回空触发调用方重新配置
  if (encrypted.startsWith('enc:')) return '';
  return EncryptionService.instance.decrypt(encrypted);
}
...
Future<Map<String, dynamic>> profileStats(Ref ref) async {
  ...
  final nodes = await ref.watch(storageLocationTreeProvider.future);
  final categories = await ref.watch(categoryManagerProvider.future);

  // 位置树里每个房间本身会产出一个 isRoom 节点，其余（柜体/箱子）即为收纳区
  final storageAreaCount = nodes.where((n) => !n.isRoom).length;
...
class AiConfigManager extends _$AiConfigManager {
  ...
  Future<AiProviderConfig> build() async {
    ...
    final activeId = await _decryptValue(await dao.getValue(_activeModelIdKey));
  ...
  Future<List<String>> _getAllModelIds(SettingsDao dao) async {
    final idsStr = await _decryptValue(await dao.getValue(_modelIdsKey));
  ...
  Future<void> _migrateLegacyData(SettingsDao dao) async {
    ...
    final legacyTypeStr = await _decryptValue(await dao.getValue(legacyTypeKey));
    ...
    if (newIds.isNotEmpty) {
      ...
      if (activeId != null) {
        await dao.setValue(_activeModelIdKey, await _encryptValue(activeId));
      }
    }
  }
  ...
  Future<AiProviderConfig> saveConfig({
    ...
    if (setAsActive) {
      await dao.setValue(_activeModelIdKey, await _encryptValue(id));
  ...
  Future<void> setActiveModel(String id) async {
    ...
    await _dao.setValue(_activeModelIdKey, await _encryptValue(id));
    state = AsyncData(config);
  }
  ...
  Future<void> deleteModel(String id) async {
    ...
    if (currentActiveId == id) {
      if (ids.isNotEmpty) {
        await dao.setValue(_activeModelIdKey, await _encryptValue(ids.first));
