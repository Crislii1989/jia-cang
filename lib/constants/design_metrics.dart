...
/// 全应用「设计稿等比缩放」工具（2026-09-16 由 `HomeMetrics` 泛化而来）。
///
/// **为什么需要**：设计稿 `docs/ui-hifi-mockups.html` 的机型是 **320×660**
/// （`.phone` 的 `padding 10/12/8` + `border 2` ⇒ 内容宽 **292**）。
/// 稿子里的尺寸（分类圆 56、统计卡内距 5/10/11、沉浸大图 158、信息行 40、
/// 表单行 44、字号 20/12.5/10.5 …）全是按这个宽度定下的**绝对值**；
/// 而运行时横向容器一律流式铺满视口。于是视口一宽（Web / 桌面预览常见 430~531），
/// **横向被拉伸、纵向纹丝不动** —— 卡片由竖版变横版、圆周围留白翻数倍、
/// 相对字号变小，和参考图「看着就不一样」。
///
/// **解法**：以「可用行宽 ÷ 设计行宽 292」为缩放因子 [k]，
/// 页面尺寸一律写成 `设计值 * k`，任何宽度都保持设计稿的比例；
/// 再配合 [maxContentWidth] 在宽视口把内容居中限宽，避免桌面端被无限放大。
///
/// 已接入：[AppCanvas]（限宽）、首页四个区块、物品详情页、添加物品页。
/// 新增页面照这个模式接：`final k = DesignMetrics.of(context);` 然后所有
/// 设计稿绝对值都 `* k`。
class DesignMetrics {
  DesignMetrics._();
