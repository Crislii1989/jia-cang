# 手机端使用指引（PWA / GitHub Pages）

> 面向「家藏」自用场景：不买开发者账号、不上架应用商店，用一条网址把 App 装到手机上。
> 线上地址：**https://crislii1989.github.io/jia-cang/**
> 更新记录：2026-09-17 首次打通（commit23 部署工作流 + commit24 工作流加固，部署 run 35219250161 success）
...
### iPhone / iPad
1. 用 **Safari** 打开 https://crislii1989.github.io/jia-cang/
   （必须是 Safari；微信内置浏览器、Chrome iOS 不支持「添加到主屏幕」）
...
| WebDAV 目录 | 新版本用 `/jiacang_backups`（旧版 `/shiwuji_backups` 的备份仍可被识别） |
...
代码改动 push 到 `main` 后，GitHub Actions 会**自动**重新构建并部署（约 3~5 分钟）：

```bash
git push origin main      # 触发 .github/workflows/deploy-web.yml
```

进度查看：https://github.com/Crislii1989/jia-cang/actions

手机上更新：**彻底关掉再重开** PWA 即可（Service Worker 会自动拉新版本）；
若界面没变，把网址在浏览器里打开并刷新一次再回主屏打开。

> 回滚：`git revert` 或 `git reset` 后再 push，会重新部署上一版本。
...
| 现象 | 处理 |
| --- | --- |
| 白色空白页 | 多等一会儿（首次要下 4.6MB）；仍白屏就清一次浏览器缓存再开 |
