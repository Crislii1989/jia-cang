...
void main() {
  ...
  group('长期闲置 / 出借中 口径', () {
    test('闲置按登记时间满 90 天且在库', () async {
      final c = await containerOf([
        item(id: 'idle100', createdAt: today.subtract(const Duration(days: 100))),
      ...
      expect(c.read(idleItemsProvider).map((i) => i.id), ['idle100']);
  ...
  group('homeReminders 聚合与排序', () {
    test('按 逾期 → 临期 → 闲置 排序，并带出对应徽标文案', () async {
      final c = await containerOf([
        item(id: 'idle', createdAt: today.subtract(const Duration(days: 120))),
      ...
      expect(list[2].badgeText, '闲置 120 天');
    });
